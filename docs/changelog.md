...
- Offer the explorer overlays (cluster, visits, missing, …) for every enabled explorer zoom level in the map layer control, labeled by zoom level, instead of only a single fixed zoom. On the explorer tile pages the overlay for the page's own zoom level stays selected by default, and the chosen overlay strategy is remembered across pages while following each page's zoom.
...
- Add documentation article on syncing activity data between multiple computers using a cloud sync service.
...
- Add a Squadrats-compatible KML export of all explored tiles (zoom 14 and 17, plus the largest square at each zoom level) so explored tiles can be imported into the Explorer Tile Helper app.
...
- Reorder the documentation navigation by themes and topics, not by Diataxis types.
...
- The documentation navbar now has a "Docs" entry that leads directly into the documentation, replacing the redundant "Home" link that duplicated the logo.
...
- Switch to **Gunicorn as the default HTTP server**. Gunicorn uses pre-fork multiprocessing with `gthread` workers, giving true parallelism across CPU cores. Control the process count with `--workers` (default: 4) and the thread count per process with `--threads` (default: 8). Waitress and Werkzeug remain available via `--http-server waitress/werkzeug`. The activity importer now also runs synchronously before the server starts, which is a prerequisite for multi-process servers.
...
- Truncate long **upstream IDs** on the activity detail page with a CSS ellipsis so they no longer overflow; the full ID is still selectable and shown in a tooltip on hover.
- Show **start time with calendar link** on the activity overview for routes with a manually set start datetime (previously only shown when GPS timestamps were present).
...
- Fix **`ValueError: max() iterable argument is empty` crash** when rendering explorer tiles at a zoom level that has no clusters yet. `MaxClusterColorStrategy` now treats the empty-clusters case as having no max-cluster members, and the dead `max()` call in `ColorfulClusterColorStrategy` (whose result was never used) has been removed. ([GH-436](https://github.com/martin-ueding/geo-activity-playground/issues/436))
- Fix **activity start time and elapsed time ignoring the trim**: trimming the first or last points of an activity (and re-enriching) now updates `start` and `elapsed_time` from the trimmed slice, matching how distance, moving time, and start/end coordinates already behave. ([GH-434](https://github.com/martin-ueding/geo-activity-playground/issues/434))
...
- Recompute **Explorer tiles** once after a Strava API sync batch finishes, instead of recalculating tile history after every imported backfill activity. ([GH-349](https://github.com/martin-ueding/geo-activity-playground/issues/349))
...
- Fix **Explorer First Visit coloring** for activities with missing timestamps: tiles now use the earliest available activity time, so untimed track points no longer make a later activity appear as the first visit. ([GH-428](https://github.com/martin-ueding/geo-activity-playground/issues/428))
...
- Add Diataxis **explanation page** for the incremental explorer cluster algorithm, including hand-drawn step-by-step visuals.
...
- Add maintenance action to **refresh activity names from Strava** via paginated API calls and upstream ID matching, without a full re-import. ([GH-369](https://github.com/martin-ueding/geo-activity-playground/issues/369))
...
- When the trim bound field is empty, actually remove the trim bound.
...
- Unify Pandas datetime instances to nanosecond resolution if needed.([GH-400](https://github.com/martin-ueding/geo-activity-playground/issues/400))
