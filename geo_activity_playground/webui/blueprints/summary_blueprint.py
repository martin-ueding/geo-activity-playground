...
def make_summary_blueprint(
    ...
    def index():
        ...
        return render_template(
            ...
            tabulate_year_kind_mean_elevation_gain=tabulate_year_kind_mean_elevation_gain(
                df
            )
            .reset_index()
            .to_dict(orient="split"),
...
def plot_year_elevation_gain_cumulative(df: pd.DataFrame) -> str:
    year_cumulative = (
        ...
        .apply(
            lambda group: pd.DataFrame(
                {
                    "week": group["week"],
                    "elevation_gain": group["elevation_gain"].cumsum().round(0),
                }
            ),
            include_groups=False,
        )
    ...
    return (
        alt.Chart(
            year_cumulative, width=500, title="Cumulative Elevation Gain per Year"
        )
        .mark_line()
        .encode(
            alt.X("week", title="Week"),
